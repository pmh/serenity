var Object = {
  ...
  set: function (name, val) {
    if (name.substring) {
      ...
      this[name].type = 'property';
      return this[name];
    } else {
      var obj = name;
      for (var k in obj) {
        this[k] = ko.observable(obj[k]);
        this[k].type = 'property';
      };
      return this;
    }
  },
  ...
  //
  // `method` creates a method on your object.
  // It also accepts an optional context and will bind your methods to it if it's provided.
  //
  method: function (name, method, context) {
    this[name] = context ? method.bind(context) : method;
  },
  
  //
  // `computed` sets up a computed property. A computed property is a function that 
  // depends on other properties and will automatically update whenever it's 
  // dependencies do.
  // It also accepts an optional context and will bind your methods to it if it's provided.
  //
  computed: function (name, method, context) {
    this.method(name, method, context);
    this[name].computed = true;
  },
