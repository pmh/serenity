var Serenity = { Object: require("./object"), Inflector: require('./inflector'), Enumerable: require('./enumerable') };
...
var Application = Serenity.Object.clone(function () {
  ...
  this._configure_controller = function (name, obj) {
    ...
    if (obj.type && obj.type.match(/ArrayController$/)) {
      var model;
      if ((model = obj._model_object()))
        obj[Serenity.Inflector.singularize(name).toLowerCase()] = obj._model_object().create();
      obj[name.toLowerCase()] = ko.observableArray([]);
      obj.extend(Serenity.Enumerable, obj[name.toLowerCase()]);
    }
  };
  ...
  this.run = function () {
    ...
    this._traverse(function (name, obj) {
      if (obj) Application._render(name, obj);
    });
  };
});
